...
function EvidenceReviewCard({
  ...
  return (
    <article className="rounded-lg border border-brand-border bg-white p-4 sm:p-5">
      <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto]">
        ...
        <span className="w-fit rounded-md bg-brand-surface-strong px-3 py-2 text-sm font-semibold text-brand-primary-dark">
          {submission.status}
        </span>
      </div>

      <form className="mt-4 grid gap-3 lg:grid-cols-[160px_140px_minmax(0,1fr)_auto]" onSubmit={handleSubmit}>
        <select name="status" defaultValue="reviewed" className="h-11 w-full min-w-0 rounded-md border border-brand-border-strong bg-white px-3 text-sm outline-none focus:border-brand-primary-dark focus:ring-2 focus:ring-brand-focus">
          <option value="reviewed">Reviewed</option>
          <option value="revision_needed">Revision Needed</option>
          <option value="submitted">Submitted</option>
        </select>
        <input name="score" type="number" min="0" max="100" defaultValue={submission.score ?? ""} placeholder="Score" className="h-11 w-full min-w-0 rounded-md border border-brand-border-strong bg-white px-3 text-sm outline-none focus:border-brand-primary-dark focus:ring-2 focus:ring-brand-focus" />
        <input name="review_notes" defaultValue={submission.review_notes ?? ""} placeholder="Catatan review" className="h-11 w-full min-w-0 rounded-md border border-brand-border-strong bg-white px-3 text-sm outline-none focus:border-brand-primary-dark focus:ring-2 focus:ring-brand-focus" />
